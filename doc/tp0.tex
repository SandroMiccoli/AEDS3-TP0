...
\subsection{Scripts}
Primeiro, foi criado um script, em Python, para gerar arquivos de entrada para o trabalho. Esse programa recebia como parâmetro o número de instâncias a ser criado e o número máximo de linhas e colunas que essas matrizes teriam. Posteriormente foi feito outro script para gerar um lotes de arquivos de entrada e já armazenar o tempo de processamento de cada um.

A seguir os dois scripts:
...
O problema de calcular o produto de Kronecker das matriz foi um problema sem muitas complicações. As matrizes foram alocadas dinâmicamente com sucesso, de acordo com o \textit{Valgrind}. Além disso, um padrão de código bem modularizado foi seguido, para que os módulos possam ser reutilizados futuramente.
...
Como primeiro trabalho da disciplina, acredito que seus objetivos foram cumpridos. As primitivas básicas da linguagem C, como alocação dinâmica de memória e manipulação de arquivos, foram bem exploradas no trabalho prático.
